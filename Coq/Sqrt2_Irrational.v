Require Import Classical_Prop Omega BasicArithmetic Arith.
(* Importing Arith allows for the `ring` tactic, which greatly simplifies addition/multiplication. *)
...
Proof.
  ...
  contradiction.
  apply (weak_sqrt2_is_irrational x x0 H1 x0_neq_0). exact H. exact H. ring.
  rewrite Nat.mul_comm. rewrite Nat.mul_assoc. rewrite Nat.mul_assoc.
  rewrite Nat.mul_comm in H2. rewrite H2. rewrite Nat.mul_shuffle0.
  rewrite <- Nat.mul_assoc. rewrite H2. ring.
